#ifndef MJPEG_DECODER_STAGE_H
#define MJPEG_DECODER_STAGE_H
...
/*
 * Value-only bridge used by the board wrapper. Keeping ownership sync here
 * makes every success/failure return host-testable without depending on ESP.
 */
typedef struct {
    uint8_t *input_buffer;
    size_t input_buffer_size;
    ...
    uint32_t canvas_height;
    uint32_t output_format;
} mjpeg_decoder_owner_state_t;

/*
 * Synchronous decoder resource stage. Frame claiming, cache locks, playback
 * timing, presentation, callbacks and WDT breadcrumbs remain in the player
 * wrapper so this component stays host-testable and platform-independent.
 */
