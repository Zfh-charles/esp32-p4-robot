...
static esp_err_t EnableI2sChannel(i2s_chan_handle_t handle) {
    if (handle == nullptr) {
        return ESP_OK;
    }
...
static esp_err_t DisableI2sChannel(i2s_chan_handle_t handle) {
    if (handle == nullptr) {
        return ESP_OK;
    }
...
void BoxAudioCodec::EnterCaptureMode() {
    std::lock_guard<std::mutex> lock(data_if_mutex_);
    if (output_enabled_) {
        ESP_ERROR_CHECK(esp_codec_dev_close(output_dev_));
        AudioCodec::EnableOutput(false);
    }
    ESP_ERROR_CHECK(DisableI2sChannel(tx_handle_));
    ESP_ERROR_CHECK(DisableI2sChannel(rx_handle_));
    ESP_ERROR_CHECK(EnableI2sChannel(rx_handle_));
    ...
    ESP_LOGI(TAG, "EnterCaptureMode (RX on, TX off)");
#if CONFIG_USE_REMINDER_POLL
    ReminderHwTraceRoute("Capture", input_enabled() ? 1 : 0, output_enabled() ? 1 : 0,
                         "EnterCaptureMode", 1);
#endif
}

void BoxAudioCodec::EnterPlaybackMode() {
    std::lock_guard<std::mutex> lock(data_if_mutex_);
    if (input_enabled_) {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    ...
    ESP_ERROR_CHECK(DisableI2sChannel(rx_handle_));
    ESP_ERROR_CHECK(DisableI2sChannel(tx_handle_));
    ESP_ERROR_CHECK(EnableI2sChannel(tx_handle_));
    ...
    ESP_LOGI(TAG, "EnterPlaybackMode (TX on, RX off)");
#if CONFIG_USE_REMINDER_POLL
    ReminderHwTraceRoute("Playback", input_enabled() ? 1 : 0, output_enabled() ? 1 : 0,
                         "EnterPlaybackMode", 1);
#endif
}

void BoxAudioCodec::EnterDuplexMode() {
    std::lock_guard<std::mutex> lock(data_if_mutex_);
    ESP_ERROR_CHECK(EnableI2sChannel(tx_handle_));
    ESP_ERROR_CHECK(EnableI2sChannel(rx_handle_));
    ...
    if (!output_enabled_) {
        ...
        ESP_ERROR_CHECK(esp_codec_dev_open(output_dev_, &out_fs));
        ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, output_volume_));
        AudioCodec::EnableOutput(true);
    }
    ESP_LOGI(TAG, "EnterDuplexMode (TX+RX on)");
#if CONFIG_USE_REMINDER_POLL
    ReminderHwTraceRoute("Duplex", input_enabled() ? 1 : 0, output_enabled() ? 1 : 0,
                         "EnterDuplexMode", 1);
#endif
}
...
void BoxAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable && input_enabled_) {
        ESP_LOGW(TAG, "EnableOutput while input active — switching RX off before TX");
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
        AudioCodec::EnableInput(false);
        ESP_ERROR_CHECK(DisableI2sChannel(rx_handle_));
        ESP_ERROR_CHECK(DisableI2sChannel(tx_handle_));
        ESP_ERROR_CHECK(EnableI2sChannel(tx_handle_));
    } else if (enable && !input_enabled_) {
        ESP_ERROR_CHECK(DisableI2sChannel(rx_handle_));
        ESP_ERROR_CHECK(EnableI2sChannel(tx_handle_));
    }
    if (enable) {
...
#if CONFIG_USE_REMINDER_POLL
        ReminderHwTraceSpeakerPcm(samples, "codec_write");
#endif
    }
    return samples;
}