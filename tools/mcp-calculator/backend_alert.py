...
配合 mcp_pipe.py 使用，需同时启动 inference API（见 xiaozhi 项目 tools/inference-api-demo/server.py）
...
def backend_get_latest_alert() -> dict:
    ...
    Use when the user says 查提醒, or asks about meetings, falls, or important reminders."""
    # Always fetch fresh — proactive wake may happen seconds after push, before the 30s poll.
    _fetch_pending()
    with _CACHE_LOCK:
...
def backend_push_alert(
    ...
    """Push a reminder into the shared inference API queue for device standby TTS."""
    try:
        payload = {
            "title": title,
            "prompt": message,
        ...
        if speak:
            payload["delivery_mode"] = "mcp_wake"
            payload["wake_text"] = os.environ.get("INFERENCE_WAKE_TEXT", "查提醒")
        else:
            payload["delivery_mode"] = "alert_only"
        result = _http_json("POST", _url("push"), payload)
    except (urllib.error.URLError, json.JSONDecodeError, TimeoutError) as e:
...
def backend_ack_alert(alert_id: str) -> dict:
    """Acknowledge a reminder in the shared inference API."""
    try:
