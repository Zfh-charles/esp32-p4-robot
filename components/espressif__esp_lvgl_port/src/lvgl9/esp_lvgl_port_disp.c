...
#if LVGL_PORT_HANDLE_FLUSH_READY
/* Optional application hook.  The final LCD transfer completion is stronger
 * than LV_EVENT_FLUSH_FINISH, which only brackets the asynchronous flush_cb. */
extern void AfeFetchGateNoteDisplayFrameReady(void) __attribute__((weak));

static bool lvgl_port_flush_io_ready_callback(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_io_event_data_t *edata, void *user_ctx)
{
    lv_display_t *disp_drv = (lv_display_t *)user_ctx;
    assert(disp_drv != NULL);
    if (lv_disp_flush_is_last(disp_drv) && AfeFetchGateNoteDisplayFrameReady) {
        AfeFetchGateNoteDisplayFrameReady();
    }
